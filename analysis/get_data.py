...
REMOTE_DATA_DIR = "/home/pi/PiCameraArray/data"
LOCAL_BASE_DIR = Path.cwd() / "collected_data"
...
def get_latest_directory(pi_name):
    """
...
def copy_data_from_pi(pi_name, remote_dir_name, local_dir):
    """
...
def copy_missing_files_from_pi(pi_name, remote_dir_name, local_dir_path):
    """
...
def count_remote_png_files(pi_name, remote_dir_name):
    """
...
def is_copy_complete(pi_name, remote_dir_name, local_dir_path):
    """
    ...
    """
    remote_count = count_remote_png_files(pi_name, remote_dir_name)
...
def main():
    ...
    e00_latest = get_latest_directory("e00")
    ...
    for pi_name in PI_NAMES:
        ...
        if pi_name in already_copied:
            ...
            if local_pi_dir:
                ...
                is_complete, remote_count, local_count = is_copy_complete(
                    pi_name, remote_dir_name, local_pi_dir
                )
                ...
                if is_complete:
                    ...
                    if copy_missing_files_from_pi(pi_name, remote_dir_name, local_pi_dir):
                        successful += 1
                        partial += 1
                    else:
                        failed += 1
        ...
        latest_dir = get_latest_directory(pi_name)
        
        if latest_dir:
            # Copy the data
            if copy_data_from_pi(pi_name, latest_dir, local_collection_dir):
                successful += 1
            else:
                failed += 1
        else:
